...
class LossComputeBase(nn.Module):
    ...
    def _stats(self, loss, scores, target, increase_ratio, n):
        """
        Args:
...
class NMTLossCompute(LossComputeBase):
    ...
    def __init__(self, generator, tgt_vocab, normalization="sents",
                 label_smoothing=0.0, tm_lambda=0.1):
    ...
    def _compute_loss(self, batch, output, target):
        scores = self.generator(self._bottle(output.contiguous()))
        ...
        stats = self._stats(loss_data, scores.data, target.view(-1).data, 0, 1)
        return loss, stats
